...
public class Maze extends JPanel {
    ...
    Maze(int[][] array, int width, int height){ // non-invariant, only for tests
        maze_wall = array;
        this.width = width;
        this.height = height;
    }

    public int getAccsess(int str, int stb){ // for tests
        return maze_wall[str][stb];
    }
...
//    }
    ...
    void clear(){ //очищение поля полностью
        ...
        if (canvas != null) canvas.paint_canvas();//прорисует начальное поле
    }
    ...
    void go_maze(){ //метод построения маршрута в лабиринте
        try {
            ...
            if(maze_wall[0][0]!=2) {
                JOptionPane.showMessageDialog(this, "Лабиринт непроходим");
            }
    ...
    void Find_way_out(int x, int y) throws InterruptedException{ //метод поиска пути
        if(x==0 && y==0){
            ...
            NewJFrame.textf.append("Dest Found\n");// для вывода пути на экрана
            NewJFrame.textf.update(NewJFrame.textf.getGraphics());// для вывода пути на экрана
            return; //если дошли до конца(выхода)
        }
        ...
        if (check_left(x,y)){
            maze_wall[x-1][y]=2;
            NewJFrame.textf.append("Go Left\n");// для вывода пути на экрана
            NewJFrame.textf.update(NewJFrame.textf.getGraphics());// для вывода пути на экрана
        ...
        if (check_down(x,y)){
            maze_wall[x][y+1]=2;
            NewJFrame.textf.append("Go Down\n");// для вывода пути на экрана
            NewJFrame.textf.update(NewJFrame.textf.getGraphics());// для вывода пути на экрана
        ...
        if (check_right(x,y)){
            maze_wall[x+1][y]=2;
            NewJFrame.textf.append("Go right\n");// для вывода пути на экрана
            NewJFrame.textf.update(NewJFrame.textf.getGraphics());// для вывода пути на экрана
            canvas.draw_rect(x + 1,y,Color.BLACK,Color.GREEN);
            Find_way_out(x+1,y);
            return;
        ...
        if (stack.size()>1){
            ...
            NewJFrame.textf.append("Go Back\n");// для вывода пути на экрана
            NewJFrame.textf.update(NewJFrame.textf.getGraphics());// для вывода пути на экрана
